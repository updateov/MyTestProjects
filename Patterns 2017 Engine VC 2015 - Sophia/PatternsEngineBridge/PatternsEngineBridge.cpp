...
namespace PatternsEngineBridge
{
	...
	cli::array<String^>^ EngineBridge::ActiveVisits::get()
	{
		CStringList* result = EngineManager::GetManager()->GetActiveVisits();
		
		cli::array<String^>^ managedArray = gcnew cli::array<String^>(result->GetCount());
	...
	int EngineBridge::EngineProcessData(String^ visitKey, cli::array<byte>^ ups, int upPosition, int upBlockSize , cli::array<byte>^ hrs, int hrPosition, int hrBlockSize)
	{
