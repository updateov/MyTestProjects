	...
	bool CheckLicense()
	{
		...
		try
		{
			...
			if (m_SSptr == NULL)
			{
				...
				if (m_SSptr == NULL)
				{
					Trace1(EVENTLOG_ERROR_TYPE, CString("Failed to load the license validation engine, contact PeriGen Support (error reference 8801)"));
					return false;
				}
				...
				{
					delete m_SSptr;
					Trace1(EVENTLOG_ERROR_TYPE, CString("Failed to initialize the license validation engine, contact PeriGen Support (error reference 8802)"));
					return false;
				}
				...
				{
					delete m_SSptr;
					Trace1(EVENTLOG_ERROR_TYPE, CString("Incorrect version of the license validation engine, contact PeriGen Support (error reference 8803)"));
					return false;
				}
			...
			{
				Trace1(EVENTLOG_ERROR_TYPE, CString("The license has expired."));
				return false;
			}
			...
			case 99:
			{
				Trace1(EVENTLOG_ERROR_TYPE, CString("Your date system has been moved back - License blocked."));
				return false;
			}
			...
			Trace1(EVENTLOG_ERROR_TYPE, CString("No license found on that computer."));
			return false;
		}
		catch (...)
		{
			Trace1(EVENTLOG_ERROR_TYPE, CString("Error while validating the license."));
			return false;
		}
	}
