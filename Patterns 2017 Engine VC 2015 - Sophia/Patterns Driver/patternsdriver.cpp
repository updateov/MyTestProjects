	...
	bool EngineCheckLicense()
	{
		...
		try
		{
			...
			if (m_SSptr == NULL)
			{
				...
				if (m_SSptr == NULL)
				{
					Trace(EVENTLOG_ERROR_TYPE, CString("Failed to load the license validation engine, contact PeriGen Support (error reference 8801)"));
					return false;
				}
				...
				{
					delete m_SSptr;
					Trace(EVENTLOG_ERROR_TYPE, CString("Failed to initialize the license validation engine, contact PeriGen Support (error reference 8802)"));
					return false;
				}
				...
				{
					delete m_SSptr;
					Trace(EVENTLOG_ERROR_TYPE, CString("Incorrect version of the license validation engine, contact PeriGen Support (error reference 8803)"));
					return false;
				}
			...
			{
				Trace(EVENTLOG_ERROR_TYPE, CString("The license has expired."));
				return false;
			}
			...
			switch (m_SSptr->SS_TrialMode())
			{
				...
				case 99:
				{
					Trace(EVENTLOG_ERROR_TYPE, CString("Your date system has been moved back - License blocked."));
					return false;
				}
			...
			Trace(EVENTLOG_ERROR_TYPE, CString("No license found on that computer."));
			return false;
		}
		catch (...) 
		{ 
			Trace(EVENTLOG_ERROR_TYPE, CString("Error while validating the license."));
			return false;
		}
	}
