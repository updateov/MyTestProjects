...
BOOL ContainerBaseWnd::Create(LPCRECT rect, HWND hParent)
{
	...
	if(pParent != NULL)
	{
		...
		CRect ctrlRect(rect);
		return CWnd::Create(PATTERNS_CTL_CLASSNAME, NULL, WS_CHILD | WS_VISIBLE, ctrlRect, pParent, 0);
	}
	return FALSE;
}


void ContainerBaseWnd::SetInitialData(const CString& url, const CString& patientID)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	m_criHWND = NULL;
...
void ContainerBaseWnd::set_data_status(data_status_code status, string msg)
{
...
int ContainerBaseWnd::calculate_required_tracing_width(int height)
{
...
int ContainerBaseWnd::calculate_required_tracing_height(int width)
{
	static int line_height = -1;
...
bool ContainerBaseWnd::GetExportCalculatedEntries(DATE from, DATE to, long id, ExportEntriesStruct& entries)
{
	if(m_Tracing != NULL)
	{
		m_Tracing->DoOnExportDlgCalculatedEntriesRequestEx(from, to, entries.m_meanContructions,  entries.m_meanBaseline,  entries.m_meanBaselineVariability,  entries.m_montevideo);		
		return true;
	}
	return false;
}
