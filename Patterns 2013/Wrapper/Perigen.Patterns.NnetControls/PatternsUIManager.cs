    ...
    public class PatternsUIManager
    {
        ...
        public void StartExport(DateTime startTime, int timeRange)
        {
            if (CRIHandle == IntPtr.Zero) //Patterns
            {
                ...
                {
                    MessageBoxWindow msgBox = new MessageBoxWindow();
                    msgBox.MessageTitle.Text = "User Login Required";
                    msgBox.MessageDescription.Text = "User login required to perform export.";
                }
            }
            else //CheckList
            {
            ...
            foreach (BaseExportEntity entity in interval.Concepts)
            {
                switch (entity.Name)
                {
                    ...
                    case "MeanBaselineVariability":
                        ((ExportDoubleEntity)entity).Value = m_meanBaselineVariability;
                        ((ExportDoubleEntity)entity).OriginValue = m_meanBaselineVariability;
                       break;
                }
            }
        ...
        public void SetExportDialogParams(string meanContractionInterval, string meanBaseline, string meanBaselineVariability, string montevideoUnits)
        {
            double dVal;

            dVal = Double.Parse(meanContractionInterval);

            if (dVal >= 0)
            {
                m_meanContractionInterval = dVal;
            }
            else
            {
                m_meanContractionInterval = null;
            ...
            dVal = Double.Parse(meanBaseline);

            if (dVal >= 0)
            {
                m_meanBaseline = (int)dVal;
            }
            else
            {
                m_meanBaseline = null;
            ...
            dVal = Double.Parse(meanBaselineVariability);

            if (dVal >= 0)
            {
                m_meanBaselineVariability = dVal;
            }
            else
            {
                m_meanBaselineVariability = null;
            ...
            dVal = Double.Parse(montevideoUnits);

            if (dVal >= 0)
            {
                m_montevideoUnits = (int)dVal;
            }
            else
            {
                m_montevideoUnits = null;
