...
BEGIN_MESSAGE_MAP(PatternsChartCtrl, COleControl)
	...
	ON_MESSAGE(GET_EXPORT_ENTRIES_MSG, OnGetExportCalculatedEntries)
...
{	
		...
		/*&& m_pPatternsAdapter != NULL && m_pPatternsAdapter->IsInitialized()*/)
	{
		...
		const UINT visible = SWP_SHOWWINDOW | SWP_NOCOPYBITS | SWP_FRAMECHANGED;
		const UINT hidden = SWP_HIDEWINDOW | SWP_NOMOVE | SWP_NOSIZE;
		m_exportBar.ShowWindow(!m_Tracing->is_zoomed() && show && isConnected && m_bExportEnabled? SW_SHOW : SW_HIDE);
...
{	
	//if(m_gaTotalDays != 0)
	//{
	//	int delta = m_gaTotalDays - MIN_WEEKS_IN_DAYS;
	//	date now;
	//	time ( &now );

	//	now -= delta * 86400;
	//	now += 60;
	//	struct tm * startDate = localtime( &now );
	//	startDate->tm_hour = 0;
	//	startDate->tm_min = 0;
	//	startDate->tm_sec = 0;
	//	date res = mktime ( startDate );
...
long PatternsChartCtrl::OnGetExportCalculatedEntries(WPARAM wParam, LPARAM lParam)
{
	if(!m_bExportEnabled)
		return 0;
	...
	if(pChunk && m_Tracing)
	{
		...
		m_Tracing->DoOnExportDlgCalculatedEntriesRequest(from, to, resultString);
		*pStr = resultString;
	}
	return 0;
}
