...
double tracing::CalculateMontevideo(long iLeft, long iRight) const
{
	...
	double montevideo = (600 * totPeak) / (double)get_displayed_length();
	return montevideo;
...
void tracing::draw_montevideo_units(CDC* dc) const
{
	...
	CRect bounds = get_bounds(otracing);
	long iright = get_index_from_x(bounds.right);
	long ileft = iright - (get_displayed_length() * get_sample_rate());
...
void tracing::draw_baseline_average(CDC* dc) const
{
	...
	if (mean_baseline > 0)
	{
		sprintf(summaryText, "Mean Baseline : %ld bpm", (long)(floor(mean_baseline)));
	}
	else
	{
	...
	if (mean_var > 0)
	{
		sprintf(summaryText, "Mean Baseline Variability: %.1f", mean_var);
	}
	else
	{
...
CRect tracing::get_bounds(object k) const
{
	...
	case oslider:
		if (has_parent_for_view())
		{
			...
			rect.left = get_x_from_index(get_index_from_ms(curTracing->get_left_in_ms()));
			rect.right = get_x_from_index(get_index_from_ms(curTracing->get_right_in_ms()));
		}
		break;
...
// =======================================================================================================================
void tracing::move_page(bool i)
{
	CRect rect = get_bounds(otracing);

	if (i)
	{
		move_to(get_index_from_x(rect.left - rect.Width()));
	}
	else
	{
		move_to(get_index_from_x(rect.left + rect.Width()));
...
// =======================================================================================================================
void tracing::move_to(long i1, long i2)
{
	stop_animating();
	if (i2 < 0 || i1 == i2)
	{
...
// =======================================================================================================================
void tracing::move_to_animating(long dl, long dr, long t, long step)
{
	if (is_animating())
	{
...
// =======================================================================================================================
void tracing::set_lengths(long le0, long lc0)
{
...
void tracing::priv_set_scaling(double s0) const
{
	if (m_scaling != s0)
	{
		if (offset != 0 && !is_scrolling())
		{
			...
			const_cast<tracing*>(this)->move_to(o);
		}
		else
		{
			m_scaling = s0;
...
void tracing::DrawExportedAreaMarker(CDC* dc, long ksubviewIndex) const
{
		...
		!ksubviews[ksubviewIndex]->is_compressed() && ksubviews[ksubviewIndex]->get_type() == tup && (long)ksubviews.size() > ksubviewIndex+1)
	{
		...
		CRect colorRect(leftP, upRect.bottom + 1, rightP, compressedRect.top - dY);			
...
void tracing::MoveToTime(date endTime)
{
	CRect rect = get_bounds(otracing);

	long iLeft = get_index_from_x(rect.left);
	long iRight = get_index_from_x(rect.right);
	long long msDestination = (long)((endTime - get()->get_start_date()) - 10) * 1000;
	...
	//GetExportDlgCalculatedEntries(ileft, iright, meanContructions, meanBaseline, meanBaselineVariability, montevideo);

	
	long long difference = msDestination - get_ms_from_index(iRight);

	if(abs(difference) < pageLengthInMs * 1.5)
	{		
		move_to_animating(iNewLeft, -1, get(kanimationduration));
	}
	else
	{
		move_to(iNewLeft);
...
void tracing::GetCompressedViewBoundsInSeconds(long& left, long& right)
{
	...
	if(get_type() == tnormal && IsWCRVisible())
	{
...
void tracing::GetExportDlgCalculatedEntries(long iLeft, long iRight, CString& meanContructionsStr, CString& meanBaselineStr, CString& meanBaselineVariabilityStr, CString& montevideoStr)
{
	double meanContructions = -1.0;
	double meanBaseline = -1.0;
	double meanBaselineVariability = -1.0;
	...
	get()->get_mean_baseline(iLeft, iRight, &meanBaseline, &meanBaselineVariability);
	...
	meanBaselineVariabilityStr.Format("%.1f", meanBaselineVariability);
	montevideoStr.Format("%.0f", montevideo);
}
void tracing::GetExportDlgCalculatedEntriesEx(long iLeft, long iRight, double& meanContructions, double& meanBaseline, double& meanBaselineVariability, double& montevideo)
{
...
bool tracing::DoOnExportDlgCalculatedEntriesRequest(DATE intervalStart, DATE intervalEnd, CString& resultString)
{
	resultString.Empty();
	...
	if(fetus)
	{
		...
		long long msDestination = (long)((exportEndTime - get()->get_start_date()) - 10) * 1000;
		...
		if(exportInterval > 15)
		{
			...
			long ileft = iNewLeft;		
			long iright = get_index_from_ms(msDestination);
			CString meanContructionsStr;
			CString meanBaselineStr;
			CString meanBaselineVariabilityStr;
			CString montevideoStr;
			GetExportDlgCalculatedEntries(ileft, iright, meanContructionsStr, meanBaselineStr, meanBaselineVariabilityStr, montevideoStr);
			resultString.Format("%s|%s|%s|%s", meanContructionsStr, meanBaselineStr, meanBaselineVariabilityStr, montevideoStr);
			return true;
...
bool tracing::DoOnExportDlgCalculatedEntriesRequestEx(DATE intervalStart, DATE intervalEnd, double& meanContructions, double& meanBaseline, double& meanBaselineVariability, double& montevideo)
{
	...
	if(fetus)
	{
		...
		long long msDestination = (long)(exportEndTime - 10) * 1000;
		int exportInterval = (int)(((long)highlightedExportTime - (long)highlightedstartTime)/60);
		if(exportInterval > 15)
		{
			long pageLength = get_expanded_length();
			...
			/*CString str;
			str.Format("%f, %f, %f, %f", meanContructions, meanBaseline, meanBaselineVariability, montevideo);
			MessageBox(str);*/
			return true;
