...
//            {
                try
                {
                    CurveURLParameters curveParameters = new CurveURLParameters();
            ...
            //{
            //    //debug
            //    string[] query = "service=http://localhost:7802/PatternsDataFeed/&curve_client_refresh=5&banner=0&curve_review_mode_enabled=False&PODI_send_epidural=False&PODI_send_vbac=False&PODI_send_vaginaldelivery=False&user_name=PeriGen Support&user_id=PeriGen&can_modify=False&can_print=True&visit_key=1-14-1-1".Split('&');
            //    foreach (var item in query)
            //        QueryString.Add(item.Split('=')[0], item.Split('=')[1]);
