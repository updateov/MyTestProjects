...
namespace CurveChartControl
{
    public class CurveURLParameters
    {
        ...
        private string curveHostName = string.Empty;
        public string CurveHostName
        {
            get { return curveHostName; }
            set { curveHostName = value; }
        }

        private int curvePort = -1;
        public int CurvePort
        {
            get { return curvePort; }
            set { curvePort = value; }
        }
        ...
        public string GetServiceURL()
        {
            ...
            if (!string.IsNullOrWhiteSpace(curveURL))
                sb.Append(string.Format("service={0}", curveURL));
            else
                sb.Append(string.Format("service=http://{0}:{1}/PatternsDataFeed/", curveHostName, curvePort));
