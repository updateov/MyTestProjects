...
namespace CurveChartControl
{
    ...
    public partial class CurveControl : UserControl
    {
        ...
        private const string m_helpFilePeriWatch = @".\Help\PeriWatch Curve User Guide.pdf";
        private const string m_helpFilePeriCALM = @".\Help\PeriCALM Curve User Guide.pdf";
        ...
        public delegate CurveURLParameters GetCurveURLParametersDelegate();
        public GetCurveURLParametersDelegate GetCurveURLParameters;
        CurveURLParameters curveParameters;
...
//            {
            try
            {
                    curveParameters = new CurveURLParameters();
                    if (GetCurveURLParameters != null)
                        curveParameters = GetCurveURLParameters();

                    string[] query = curveParameters.GetServiceURL().Split('&');
                    ...
                    DataCallRequest = new XElement("patients", "{0}", new XAttribute("version", curveParameters.CurveVersion), new XAttribute("user", QueryString[queryStringUserId] ?? string.Empty)).ToString(SaveOptions.DisableFormatting);
                }
                catch (Exception ex)
                {
                    //No parameters			
            ...
            //{
            //    //debug
            //    string[] query = "service=http://localhost:7802/PatternsDataFeed/&curve_client_refresh=5&banner=0&curve_review_mode_enabled=False&PODI_send_epidural=False&PODI_send_vbac=False&PODI_send_vaginaldelivery=False&user_name=PeriGen Support&user_id=PeriGen&can_modify=False&can_print=True&visit_key=2-158-1-1".Split('&');
            //    foreach (var item in query)
            //        QueryString.Add(item.Split('=')[0], item.Split('=')[1]);
		...
		void btnSnapshots_Click(object sender, RoutedEventArgs e)
		{
            ...
            var result = SendRequest(ServiceURL + "snapshots", string.Format(DataCallRequest, stringLastRequest), "POST");
			...
			foreach (var item in data.Descendants("snapshot"))
			{
				...
				if (liveOne)
				{
                    ...
                    menuItem.Icon = new Image() { Source = new BitmapImage(new Uri(BaseUriHelper.GetBaseUri(this), @"/CurveChartControl;component/Images/lastSnapshot.png")) };
		...
		void btnAbout_Click(object sender, RoutedEventArgs e)
		{
            ...
            bool isPeriCALM = logo == 0;
            var aboutData = new AboutData(isPeriCALM);
            //if (isPeriCALM)
            //{
            ...
            //    }
            //    periCALMLogo.Visibility = Visibility.Visible;
            //    periWatchLogo.Visibility = Visibility.Collapsed;
            //}
            //else
            //{
            //    periWatchLogo.Visibility = Visibility.Visible;
            //    periCALMLogo.Visibility = Visibility.Collapsed;
            //}
        ...
        private int GetLogoParameter()
        {
            try
            {
                if (curveParameters.Logo != -1)
                    return curveParameters.Logo;
                else
                {
                    string result = SendRequest(ServiceURL + "productinfo", null, "GET");
                    if (result.Length == 2) // 2 because of the + "\r\n"
                        throw new Exception("Unable to reach the server");

                    XElement productNode = XElement.Parse(result, LoadOptions.None);
                    XAttribute logoAttribute = productNode.Attribute("logo");
                    return int.Parse(logoAttribute.Value);
                }
            }
            catch (Exception ex)
            {
        ...
        //            Resources.MergedDictionaries.Insert(i, Application.LoadComponent(new Uri("CurveChartControl;component/Themes/Theme" + currentTheme + ".xaml", UriKind.Relative)) as ResourceDictionary);
        //            break;
        //        }
        //    }
        //}
		...
		void WorkerDoWork(object sender, DoWorkEventArgs e)
		{
			...
			try
			{
				...
				if (string.IsNullOrEmpty(e.Argument.ToString()))
				{
					...
					{
                        //first time
                        e.Result = SendRequest(ServiceURL + "curve", string.Format(DataCallRequest, "<request key=\"" + QueryString.GetValueWithDefault(queryStringVisitKey) + "\" />"), "POST");
                    }
                    else
					{
                        //refresh
                        e.Result = SendRequest(ServiceURL + "curve", string.Format(DataCallRequest, stringLastRequest), "POST");
                    }
                }
				else
				{
                    //edition
                    e.Result = SendRequest(ServiceURL + "updatefields", (XElement.Parse("<data>" + e.Argument.ToString() + "</data>")).ToString(), "POST");
                }
            }
			catch (Exception ex)
			{
        ...
        static string SendRequest(string uri, string body, string requestMethod)
        {
		...
		public static readonly DependencyProperty IsEditionAllowedProperty =
            DependencyProperty.Register("IsEditionAllowed", typeof(Boolean), typeof(CurveControl), new PropertyMetadata(true));
		...
		public static readonly DependencyProperty IsReviewModeVisibleProperty =
            DependencyProperty.Register("IsReviewModeVisible", typeof(Boolean), typeof(CurveControl), new PropertyMetadata(true));
		...
		public static readonly DependencyProperty IsDemoModeProperty =
            DependencyProperty.Register("IsDemoMode", typeof(Boolean), typeof(CurveControl), new PropertyMetadata(false));
		...
		public static readonly DependencyProperty IsPoweredByLogoProperty =
            DependencyProperty.Register("IsPoweredByLogo", typeof(Boolean), typeof(CurveControl), new PropertyMetadata(false));
		...
		public static readonly DependencyProperty IsEpiduralEditionVisibleProperty =
            DependencyProperty.Register("IsEpiduralEditionVisible", typeof(Boolean), typeof(CurveControl), new PropertyMetadata(true));
		...
		public static readonly DependencyProperty IsPreviousVaginalEditionVisibleProperty =
            DependencyProperty.Register("IsPreviousVaginalEditionVisible", typeof(Boolean), typeof(CurveControl), new PropertyMetadata(true));
		...
		public static readonly DependencyProperty IsVBACEditionVisibleProperty =
			DependencyProperty.Register("IsVBACEditionVisible", typeof(Boolean), typeof(CurveControl), new PropertyMetadata(true));
        ...
        private void txtHelp_MouseLeftButtonUp(object sender, MouseButtonEventArgs e)
        {
            ...
            string helpFile = logo == 0 ? m_helpFilePeriCALM : m_helpFilePeriWatch;
        ...
        public ImageBrush ImageBrushForPosition(Exam.FetalPositionEnum position)
        {
            ...
            if (imageBrushPositions == null)
            {
                ...
                foreach (string item in Enum.GetNames(typeof(Exam.FetalPositionEnum)))
                {
                    ...
                    {
                        imageName = @"/CurveChartControl;component/Images/NoStation.png";
                    }
                    else
                    {
                        imageName = @"/CurveChartControl;component/Images/" + item + ".png";
                    }
