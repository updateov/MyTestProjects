...
namespace PeriGen.Patterns.Service
{
	public class PatternsChalkboard : IDisposable
	{
        ...
        public MothersMap Mothers = new MothersMap();
		#endregion
		...
		protected virtual void Dispose(bool disposing)
		{
			if (!this.IsDisposed)
			{
				if (disposing)
				{
                    PatternsProcessorManager.Instance.TerminateAllProcesses();
		...
		bool QueryPODI()
		{
			try
			{
				...
				try
				{
                    ...
                    //EndpointAddress addr = new EndpointAddress(PatternsServiceSettings.Instance.PODILinkURL);
                    //ChannelFactory<IPeriGenOutbound> chn = new ChannelFactory<IPeriGenOutbound>(bind, addr);
                    //IPeriGenOutbound podi = chn.CreateChannel();
                    //result = podi.GetData(this.PODIData);
                }
				catch (Exception e)
				{
		...
		void UpdateEpisode(XElement visit)
		{
			try
			{
                ...
                if (visit.Attribute("mother_id") != null)
                {
                    motherID = visit.Attribute("mother_id").Value;
                }
                ...
                if (status == PatternsEpisode.PODIVisitStatus.Merged)
                {
                    lock (EpisodesLockObject)
                    {
                        int mergeId = episode.MergeId;
                        int uniqueId = episode.PatientUniqueId;
                        String dbFile = episode.DatabaseFile;
                        episode.EpisodeStatus = PatternsEpisode.EpisodeStatuses.Closed;

                        Episodes.Remove(episode);
                        episode = new PatternsEpisode
                        {
                            Key = key,
                            EpisodeStatus = PatternsEpisode.EpisodeStatuses.Merged,
                            RecoveryStatus = PatternsEpisode.RecoveryStatuses.Curve | PatternsEpisode.RecoveryStatuses.Patterns | PatternsEpisode.RecoveryStatuses.Podi,
                            LastMonitored = DateTime.UtcNow,
                ...
                {
                    this.Mothers.AddVsitKey(motherID, key, episode.PatientUniqueId);
                }
                ...
                lock (episode.LockObject)
				{
					...
					{                       
                        ...
                        episode.BedId = data.Attribute("bed_id") != null ? data.Attribute("bed_id").Value : String.Empty;
                        episode.BedName = data.Attribute("bed_name") != null ? data.Attribute("bed_name").Value : String.Empty;
						episode.UnitName = data.Attribute("unit_name") != null ? data.Attribute("unit_name").Value : String.Empty;

						episode.MRN = data.Attribute("mrn") != null ? data.Attribute("mrn").Value : String.Empty;
                        episode.AccountNo = data.Attribute("account") != null ? data.Attribute("account").Value : String.Empty;

                        episode.FirstName = data.Attribute("first_name") != null ? data.Attribute("first_name").Value : String.Empty;
                        episode.LastName = data.Attribute("last_name") != null ? data.Attribute("last_name").Value : String.Empty;

						episode.EDD = data.AttributeToDateTime("edd");
                        episode.GA = data.Attribute("ga") != null ? data.Attribute("ga").Value : String.Empty;

						episode.Fetuses = data.AttributeToInteger("fetuses");
                        episode.Real_Fetuses = (data.Attribute("real_fetuses") != null)? data.AttributeToInteger("real_fetuses") : null;
                        ...
                        episode.PreviousVaginal = !data.AttributeToBoolean("first_vaginal");

                        episode.MembraneStatus = data.Attribute("membrane_status") != null ? data.Attribute("membrane_status").Value: String.Empty;
					...
					if (exams != null)
					{
						...
						foreach (var examElement in exams.Elements("exam"))
						{
							...
							if (time.HasValue)
							{
								list.Add(new DataContextEpisode.PelvicExam
									{
										Time = time.Value.ToEpoch(),

                                        Dilatation = examElement.Attribute("dilatation") != null ? examElement.Attribute("dilatation").Value : String.Empty,
                                        Effacement = examElement.Attribute("effacement") != null ? examElement.Attribute("effacement").Value : String.Empty,
                                        Station = examElement.Attribute("station") != null ? examElement.Attribute("station").Value : String.Empty,

                                        Presentation = examElement.Attribute("presentation") != null ? examElement.Attribute("presentation").Value : String.Empty,
                                        Position = examElement.Attribute("position") != null ? examElement.Attribute("position").Value : String.Empty,
        ...
        internal void AddVisitToMother(string motherID, string visitKey, int patientUniqueID)
        {
            Mothers.AddVsitKey(motherID, visitKey, patientUniqueID);
        }

        public List<EpisodeIdentifier> GetVisitsByMother(string motherID)
        {
            
            lock (this.EpisodesLockObject)
            {
               var visitsByMotherID = this.Mothers.GetMultiples(motherID);
               var visits = new List<EpisodeIdentifier>(visitsByMotherID);
               return visits;                
            }           
        }

        public XElement GetMultiples(string motherID)
        {
            lock (this.EpisodesLockObject)
            {
                return this.Mothers.GetMultiplesXElement(motherID);
            }
        }
        public XElement GetDummyMultiples(string visitKey, int patientUniqueID)
        {
            XElement multiples = new XElement("multiples");
            multiples.Add(new XAttribute("motherId", visitKey));
            XElement element = new XElement("sibling");
            element.Add(new XAttribute("PatientUniqueId", patientUniqueID.ToString(CultureInfo.InvariantCulture)));
            element.Add(new XAttribute("key", visitKey));
            multiples.Add(element);
            return multiples;
        }

        public void RemoveClosedVisitFromMother(string motherID, string visitKey)
        {
            lock (this.EpisodesLockObject)
            {
                this.Mothers.RemoveVisit(motherID, visitKey);
 
            }
        }

        public void RemoveClosedVisitsFromMothersMap()
        {
            var closedEpisodes  = this.Episodes.Where(e => e.EpisodeStatus == PatternsEpisode.EpisodeStatuses.Closed);
            foreach (var episode in closedEpisodes)
            {
                if (episode.EpisodeStatus == PatternsEpisode.EpisodeStatuses.Closed && !String.IsNullOrEmpty(episode.MotherID))
                {
                    this.Mothers.RemoveVisit(episode.MotherID, episode.Key);
                }
            }
        }
	}
