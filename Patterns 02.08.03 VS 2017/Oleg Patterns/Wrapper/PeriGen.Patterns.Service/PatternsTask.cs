...
namespace PeriGen.Patterns.Service
{
	...
	class PatternsTask
	{
        ...
        static object curveRefreshLock = new object();
		...
		public bool Start()
		{
			try
			{
				lock (this.ThreadLock)
				{
                    ...
                    PatternsProcessorManager.Instance.PatternsProcessorMaxVisits = PatternsServiceSettings.Instance.PatternsProcessorMaxVisits;
                    ...
                    foreach (var episode in episodes)
                    {
                        lock (episode.LockObject)
                        {
                            if (!string.IsNullOrEmpty(episode.MotherID))
                                Common.Chalkboard.Mothers.UpdatePatientUniqueID(episode.MotherID, episode.Key, episode.PatientUniqueId);
                        }
                    }
                }
            }
        ...
        static void ProcessDataAsync()
        {
            try
            {
                ...
                foreach (var episode in episodes)
                {
                    Task episodeTask = episode.ProcessDataTaskAsync();
                    allTasks.Add(episodeTask);

                    Task episodeContinuationTask = episodeTask.ContinueWith(new Action<Task>((antecedent) =>
                    {
                        RefreshEpisodesCurve(episodes, refreshWatch);
                    }));
                    allTasks.Add(episodeContinuationTask);
                }
        ...
        private static void RefreshEpisodesCurve(List<PatternsEpisode> episodes, Stopwatch refreshWatch)
        {
            lock (curveRefreshLock)
            {
		...
		static void LoadEpisodes()
		{
			...
			using (var db = new DataContextCatalog.DataContextCatalog(CatalogDatabaseSource))
			{
				...
				foreach (var item in openEpisodes)
				{
                    ...
                    if(!String.IsNullOrEmpty(episode.MotherID))
                    {
                        Common.Chalkboard.AddVisitToMother(episode.MotherID, episode.Key, episode.PatientUniqueId);
                    }
				}
			}
        ...
        static void PurgeClosedEpisodes()
        {
            ...
            if (Common.Chalkboard != null) // The chalkboard is null the first time this method is called since it's not loaded yet
            {
                lock (Common.Chalkboard.EpisodesLockObject)
                {
                    Common.Chalkboard.RemoveClosedVisitsFromMothersMap();
