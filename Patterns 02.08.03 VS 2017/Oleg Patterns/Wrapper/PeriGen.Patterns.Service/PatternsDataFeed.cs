...
namespace PeriGen.Patterns.Service
{
	...
	public class PatternsDataFeed : ICurveDataFeed
	{
		...
		static bool ValidatePatternsClientVersion(string version)
		{
            ...
            string clientMajorVersion = version.Length > 8 ?
               version.Substring(0, 8) : version;
            string pattersnMajorVersion = PatternsServiceSettings.Instance.VersionPatterns.Length > 8 ?
                PatternsServiceSettings.Instance.VersionPatterns.Substring(0, 8) : PatternsServiceSettings.Instance.VersionPatterns;
            ...
            if (string.CompareOrdinal(clientMajorVersion, pattersnMajorVersion) != 0)
            {
		...
		static bool ValidateCurveClientVersion(string version)
		{
            ...
            string clientMajorVersion = version.Length > 8 ?
               version.Substring(0, 8) : version;
            string curveMajorVersion = PatternsServiceSettings.Instance.VersionCurve.Length > 8 ?
                PatternsServiceSettings.Instance.VersionCurve.Substring(0, 8) : PatternsServiceSettings.Instance.VersionCurve;

            // Easy comparison, we want the EXACT version!
            //if (string.CompareOrdinal(version, PatternsServiceSettings.Instance.VersionCurve) != 0)
            if (string.CompareOrdinal(clientMajorVersion, curveMajorVersion) != 0)
            {
		...
		public XElement GetPatternsData(XElement param)
		{
			...
			{
                root.SetAttributeValue("Default15MinView", PatternsServiceSettings.Instance.Default15MinView);
                foreach (var node in param.Descendants("request"))
				{
					...
					lock (episode.LockObject)
					{
						...
						int lastMergeId = Int32.Parse(header.LastMergeId ?? "-1", CultureInfo.InvariantCulture);

                        if (lastMergeId != episode.MergeId)
                        {
                            header.LastAction = "-1";
                            header.LastArtifact = "-1";
                            header.LastTracing = "-1";
                            header.LastMergeId = episode.MergeId.ToString();
                            header.LastMergeTime = episode.MergeTime.ToString();
                            header.Incremental = false;
                        }
					...
					DateTime firstTracing = DateTime.MinValue;
					{
						...
						if (tracings != null && tracings.Count > 0)
						{
							...
							firstTracing = tracings.Min(c => c.Start); // Remember the start time of the oldest tracing block returned, used later on for the artifacts							
						}
					}
					...
					//// ARTIFACTS
					{
						...
						lock (episode.TracingArtifacts)
						{
                            ...
                            else 
                            {
                                ...
                                DateTime artifactsStartTime = episode.TracingArtifacts.Count > 0 ? episode.TracingArtifacts.Min(c => c.StartTime) : DateTime.MinValue;
        ...
        public XElement GetPatientList()
        {
            ...
            lock (Common.Chalkboard.EpisodesLockObject)
            {
                ...
                {
                    episodes = Common.Chalkboard.Episodes
                                    .Where(e => e.EpisodeStatus != PatternsEpisode.EpisodeStatuses.New && e.EpisodeStatus != PatternsEpisode.EpisodeStatuses.Closed && e.PatientUniqueId > 0)
                                    .OrderBy(e => e.EpisodeStatus)
                                    .ThenBy(e => e.UnitName)
                                    .ThenBy(e => e.BedName)
            ...
            XElement episodesElement = new XElement("Episodes");
            episodesElement.Add(new XAttribute("LastRefreshed", Common.Chalkboard.LastRefreshed.ToString("s", CultureInfo.InvariantCulture)));
            episodesElement.Add(new XAttribute("IsOffline", Common.Chalkboard.IsOffline.ToString()));
            if (episodes != null)
            {
                foreach (var episode in episodes)
                {
                    XElement episodeElement = new XElement("Episode");
                    episodeElement.Add(new XAttribute("PatientUniqueId", episode.PatientUniqueId.ToString(CultureInfo.InvariantCulture)));
                    episodeElement.Add(new XAttribute("DatabaseFile", episode.DatabaseFile ?? string.Empty));
                    episodeElement.Add(new XAttribute("EpisodeStatus", episode.EpisodeStatus.ToString()));
                    episodeElement.Add(new XAttribute("RecoveryInProgress", episode.RecoveryInProgress.ToString(CultureInfo.InvariantCulture)));
                    episodeElement.Add(new XAttribute("Created", episode.Created.ToString("s", CultureInfo.InvariantCulture)));
                    episodeElement.Add(new XAttribute("LastUpdated", episode.LastUpdated.ToString("s", CultureInfo.InvariantCulture)));
                    episodeElement.Add(new XAttribute("LastMonitored", episode.LastMonitored.ToString("s", CultureInfo.InvariantCulture)));
                    episodeElement.Add(new XElement("RealTimeLatency", episode.RealTimeLatency.ToString(CultureInfo.InvariantCulture)));
                    XElement patientElement = episode.GetPatientXElement();
                    if (!string.IsNullOrEmpty(episode.MotherID))
                    {
                        XElement multiples = Common.Chalkboard.GetMultiples(episode.MotherID);
                        patientElement.Add(multiples);
                    }
                    else
                    {
                        XElement dummy = Common.Chalkboard.GetDummyMultiples(episode.Key, episode.PatientUniqueId);
                        patientElement.Add(dummy);
                    }

                    episodeElement.Add(patientElement);
        ...
        public XElement GetArtifactByID(XElement param)
        {
            ...
            if (episode != null)
            {
                if(episode.TracingArtifacts != null)
                    artifact = episode.TracingArtifacts.FirstOrDefault(t => t.Id == action.ArtifactId);
        ...
        {
            XElement info = new XElement("product", new XAttribute("brand", PatternsServiceSettings.Instance.BrandName));
            info.Add(new XAttribute("patterns", PatternsServiceSettings.Instance.ApplicationName));
            info.Add(new XAttribute("BrandMarkTM", PatternsServiceSettings.Instance.BrandMarkTM.ToString(CultureInfo.InvariantCulture)));      
           
            info.Add(new XAttribute("logo", PatternsServiceSettings.Instance.Logo.ToString(CultureInfo.InvariantCulture)));
            return info;
        }
        #endregion
    }
