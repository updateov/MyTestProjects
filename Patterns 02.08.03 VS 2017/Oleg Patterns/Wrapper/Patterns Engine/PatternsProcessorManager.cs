...
{
    public class PatternsProcessorManager
    {
        ...
        private static object s_visitAddRemoveLock = new object();

        private Timer m_SyncChildProcessCollectionsTimer;

        internal bool InitializeVisit(string visitKey)
        {
            lock (s_visitAddRemoveLock)
            {
                ...
                toRet = AssignVisitToProcess(visitKey, processGUID);
        ...
        internal bool ProcessVisit(string visitKey, byte[] ups, byte[] hrs, int startIndex, int length, ref StringBuilder data)
        {
        ...
        private ChannelFactory<IPatternsProcessorMessages> CreateChannelFactory(string processGUID)
        {
            ...
            return new ChannelFactory<IPatternsProcessorMessages>(binding, new EndpointAddress(string.Format(PatternsProcessorPipeName, processGUID/*"e6be380e-b21a-4596-a1a2-c83241c09097"*/)));
        }
        ...
        {
            m_SyncChildProcessCollectionsTimer = new Timer(PatternsProcessorSyncTimeSec * 1000);
