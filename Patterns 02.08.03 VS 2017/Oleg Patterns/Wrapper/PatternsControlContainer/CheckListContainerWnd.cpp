...
void CheckListContainerWnd::SetInitialData(const CString& url, const CString& patientID,  const CString& userID, const CString& userName, const CString& permissions, bool exportSupport, const CString& version)
{
...
void CheckListContainerWnd::StartInitializationThread()
{
	
	try
	{		

		m_pWorkerThread = (ChecklistInitWorkerThread*)AfxBeginThread(RUNTIME_CLASS(ChecklistInitWorkerThread));
...
void CheckListContainerWnd::SetDataToWorkerThread()
{
	
	patterns::CRIFetus* f = GetFetus();
	CRIInputAdapter::patient* p = get_patient();
	
	ChecklistInitialData* pData = new ChecklistInitialData(f, p, m_ServerURL, m_patientID, &m_RequestHeader,
	...
	{
		((ChecklistInitWorkerThread*)m_pWorkerThread)->SetInitialData(pData);
	}
}