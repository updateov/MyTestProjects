...
PatternsContainerWnd::PatternsContainerWnd()
{
	...
	m_pWorkerThread = new PatternsInitWorkerThread();
	m_cr_limit = 10;
	m_cr_window = 10;
	...
	m_Tracing15MinView = new tracing();
	m_Tracing15MinView->set_can_delete(false);
	m_Tracing30MinView = new tracing();
...
void PatternsContainerWnd::SetInitialData(const CString& url, const CString& patientID,  const CString& userID, const CString& userName, const CString& permissions, bool exportSupport, const CString& version)
{
	ContainerBaseWnd::SetInitialData(url, patientID, userID, userName, permissions, exportSupport, version);
...
void PatternsContainerWnd::process_patient_data(TiXmlElement* rootNode)
{
	...
	if (incremental == 0)
	{
		...
		m_roundBaselineFHRValue = roundValue <= 0 ? 1 : roundValue;
		...
		m_roundByMontevideoUnits = roundValue <= 0 ? 1 : roundValue;
	...
	bool merge = false;
	int lastMergeID = -1;
	const char* mergeAttr = requestNode->Attribute("merge");
	if (mergeAttr != NULL)
		requestNode->Attribute("merge", &lastMergeID);
	if (m_lastMergeID != lastMergeID)
	{
		merge = (incremental || m_lastMergeID < lastMergeID);
		m_lastMergeID = lastMergeID;
	}
	...
	case ePatientLive:
		...
		set_data_status(DataStatus::data_status_connected, status_details);
		break;
	...
	case ePatientRecovery:
		...
		set_data_status(DataStatus::data_status_recovery, status_details);
		break;

	case ePatientInvalid:
		ClearPatient();
		set_data_status(DataStatus::data_status_no_data, status_details);
		break;

	case ePatientError:
	default: 
		ClearPatient();
		set_data_status(DataStatus::data_status_error, status_details);
		break;
	}
	...
	m_Tracing15MinView->show(patterns_gui::tracing::wshowdisclaimer, (m_Disclaimer > 0));
	m_Tracing30MinView->show(patterns_gui::tracing::wshowdisclaimer, (m_Disclaimer > 0));
	...
	if (incremental == 0)
	{
		...
		{
			ShowMVU(mvuIsVisible, false, false);
		}
...
void PatternsContainerWnd::display_data_status(void)
{
	...
	{
		if (m_CriticalError || (m_DataStatus == DataStatus::data_status_error))
		{
			CString t;
...
//////////////////////////////////////////////////
void PatternsContainerWnd::StartInitializationThread()
{

	try
	{

		m_pWorkerThread = (PatternsInitWorkerThread*)AfxBeginThread(RUNTIME_CLASS(PatternsInitWorkerThread));
		if (m_pWorkerThread != NULL)
		{
			WaitForSingleObject(m_pWorkerThread->m_startEvent, INFINITE);
			m_pWorkerThread->CreateThreadWnd(m_hWnd);

		}
		else
		{
			DoOnInitializationEnd(false);
		}
	}
	catch (...)
	{
...
void PatternsContainerWnd::SetDataToWorkerThread()
{
	
	patterns::fetus* f = get_fetus();;
	input_adapter::patient* p = get_patient();
	
	PatternsInitialData* pData = new PatternsInitialData(f, p, m_ServerURL, m_patientID, &m_RequestHeader,
		m_Permissions, &m_Disclaimer, &m_Banner, &m_CriticalError, &m_gaTotalDays, &m_DemoMode,
		&m_intervalDuration, &m_GA, &m_exportableIntervals, m_bExportEnabled, display_patient_banner(), m_hWnd,
		m_str_strikeout_by_label, m_str_restore_by_label, m_str_confirm_by_label, m_str_action_performed_at,
		&m_isInitialRequest, &m_isFirstRequest, &m_supportedPatternsVersions, &m_currentPatternsVersion, &m_currentVersionIndex, m_UserID);
	if (m_hThreadWnd != NULL && IsWindow(m_hThreadWnd))
	{
		((PatternsInitWorkerThread*)m_pWorkerThread)->SetInitialData(pData);
	}
}