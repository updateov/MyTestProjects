...
const int TRACING_APP_VIEW_SIZE_SEC = 1800;
const int TRACING_VIEW_SIZE_SEC = 900;//1800;//1020; //1800;
const int COMPRESS_VIEW_SIZE_SEC = 14400;//7200; //14400;
const int SLIDER_VIEW_SIZE_SEC = 1800;

#define TRACINGS_VIEW_SIZE_15_MINUTES 15
#define TRACINGS_VIEW_SIZE_30_MINUTES 30

namespace patterns_gui
{
#define SHOW_EXPORT_MSG (WM_USER + 20)
#define FORCE_UPDATE_ADAPTER_MSG (WM_USER + 21)
#define SWITCH_15MIN_30MIN_VIEWS_MSG (WM_USER + 22)
#define TOGGLE_SWITCH_MSG (WM_USER + 23)
#define TOGGLE_SWITCH_NOTIFICATION_MSG (WM_USER + 24)

#define MSG_SETSLIDERPOSITION (WM_USER + 230)
#define MSG_GOTO_SELECTEDINTERVAL (WM_USER + 231)
#define MSG_GOTO_TIME (WM_USER + 232)
	/*
	=======================================================================================================================
	...
	*/
	...
	protected:
		...
		enum object { obuttonaccept, obuttondelete, opictogram, oprimary, osecondary, oselection, oslider, osliderleft, osliderright, otracing, oview, ocompletetracing, otoggle, otoggleleft, otoggleright, ozoombtn };
		...
		virtual void DrawSummaryPanel(CDC *) const;
		...
		virtual long GetCompleteLength(void) const;
		virtual long GetExpandedViewCompleteLength() const;
		...
		virtual long long GetSliderLeftInMs(void) const;
	...
	public:
		...
		virtual long long GetSliderRightInMs(void) const;
	protected:
	...
	public:
		...
		virtual long   GetSliderLength(void) const;
		...
		virtual void move_page(bool = true, bool toggleSwith = false);
		virtual void move_page_animating(bool = true);
		virtual void move_to(long, long = -1, bool toggleSwith = false);
		virtual void move_to_animating(long, long = -1, long = -1, long = -1, bool toggleSwith = false);
		virtual void MoveToTime(date endTime, bool toggleSwith = false, bool useAnimating = true);
		void MoveCompressedViewToTime(date endTime);
		...
		virtual void set_lengths(long, long, long);
	...
	protected:
		...
		mutable bool m_bLeftPartShown;

		std::set<long> m_visibleChunkEnds;
		mutable CRect m_toggleRect;
		mutable CRect m_zoomBtnRect;
		mutable int m_logo;
	public:
	...
	public:
		bool IsLeftPartShown() const
		{
			return m_bLeftPartShown;
		}
		void SetLeftPartShown(bool shown) const
		{
			m_bLeftPartShown = shown;
		}
		void ChangeToggleButtonStateWhileSliding(CPoint p0, bool expanded);
		bool IsToggleButtonAvailable() const;
		void DrawToogleButton(CDC* dc, const CRect& sliderRect) const;
		void DrawToggleZoom(const CRect& toggleRect, CDC* dc) const;
		void ToggleSwitchLeft(bool bNotify = true);
		void ToggleSwitchRight(bool bNotify = true);
		void DrawEdge(CDC* dc) const;
		void SwitchViews();
		bool GetExpandedViewStartAndEnd(date& startTime, date& endTime);
	public:
		void ResetExportData();
		...
		bool DoOnExportDlgCalculatedEntriesRequestEx(DATE intervalStart, DATE intervalEnd, double& meanContructions, int& meanBaseline, int& meanBaselineVariability, double& montevideo, bool& contractionThresholdExceeded);
		bool MoveToInterval(DATE intervalStart, int range);
		CRect GetUpRect();
	protected:
	...
	public:
		void DisableToggleZoom()
		{
			m_bIsTogleZoomDisabled = true;
		}
		bool IsExportDlgOpenFor15MinInterval();
		void MoveToHighlited15MinExportInterval();

		virtual bool IsContractionThresholdExceeded(long iLeft, long iRight) const
		{
			return false;
		}
		virtual bool GetContractionThresholdExceeded(DATE from, DATE to);
public:
	void UpdateScaling();
protected:
	void DrawShadowRect(CDC* dc, const CRect& rect, bool blue, bool rightShadow) const;
private:
	bool m_bIsTogleZoomDisabled;
};
