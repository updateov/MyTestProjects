...
namespace PatternsDriver
{
	...
	bool CheckLicense()
	{
		...
		try
		{
			...
			//return false;
			// PAW 20171123 disable license for debugging
			return true;
		}
		catch (...)
		{
	...
	{
		Trace1(EVENTLOG_ERROR_TYPE, "PAW debug: PatternsWorkerWrapper()");
		if (!CheckLicense())
		{
			m_pPatternsWorker = NULL;
			Trace1(EVENTLOG_ERROR_TYPE, "PAW debug: PatternsWorkerWrapper() failed");
		}
		else
		{
			m_pPatternsWorker = new PatternsWorker;
			Trace1(EVENTLOG_ERROR_TYPE, "PAW debug: PatternsWorkerWrapper() passed");
		}
	}
	...
	{
		Trace1(EVENTLOG_ERROR_TYPE, "PAW debug: PatternsWorkerWrapper::GetEngine()");
	...
	{
		Trace1(EVENTLOG_ERROR_TYPE, "PAW debug: PatternsWorkerWrapper::ProcessHR() entered");
	...
	{
		Trace1(EVENTLOG_ERROR_TYPE, "PAW debug: PatternsWorkerWrapper::ProcessUP() entered");
		if (m_pPatternsWorker == 0)
			throw exception("Invalid operation, no valid engine provided");
	...
	{
		Trace1(EVENTLOG_ERROR_TYPE, "PAW debug: PatternsWorkerWrapper::ReadResults() entered");
