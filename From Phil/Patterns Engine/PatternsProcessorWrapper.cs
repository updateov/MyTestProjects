...
namespace PeriGen.Patterns.Engine
{
    ...
    public class PatternsProcessorWrapper : IDisposable
    {
        ...
        /// <summary>
        /// The current time associated to the next Process event
        /// </summary>
        public DateTime CurrentTime { get; protected set; }

        /// <summary>
        /// Engine name used for logging, optional
        /// </summary>
        string Name { get; set; }

        internal static TraceSource Source = new TraceSource("PatternsEngine");
        ...
        public PatternsProcessorWrapper(string visitKey, DateTime startTime)
            : this(visitKey, startTime, string.Empty)
        {
        }
        ...
        /// </summary>
        /// <param name="startTime"></param>
        public PatternsProcessorWrapper(string visitKey, DateTime startTime, string name)
        {
            ...
            // Trace and counters
            Source.TraceEvent(TraceEventType.Verbose, 1200, "{0} instanciation at {1:s}", this, startTime);
        ...
        /// </summary>
        ~PatternsProcessorWrapper()
        {
            Source.TraceEvent(TraceEventType.Verbose, 1201, "{0} disposed", this);
            Dispose(false);
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (!this.IsDisposed)
            {
                ...
                PatternsProcessorManager.Instance.UninitializeVisit(VisitKey);
        ...
        public IEnumerable<DetectedObject> Process(byte[] hrs, byte[] ups, int startIndex, int length)
        {
            try
            {
                ...
                lock (this.LockObject)
                {
                    ...
                    bool engineInitialized = PatternsProcessorManager.Instance.InitializeVisit(VisitKey);
                    if (!engineInitialized)
                        throw new InvalidOperationException("Unable to initialize the calculation engine");

                    StringBuilder data1 = new StringBuilder();
                    bool moreData1 = PatternsProcessorManager.Instance.ProcessVisit(VisitKey, ups, hrs, startIndex, length, ref data1);
                    ...
                    if (data1.Length > 0)
                    {
                        ...
                        foreach (string line in lines)
                        {
                            Source.TraceEvent(TraceEventType.Verbose, 1201, "PAW debug: PatternsProcessorWrapper::Process Data line: {0}", line);
                            var artifact = line.ToDetectionArtifact(this.StartTime);
                            if (artifact != null)
                            {
                                results.Add(artifact);
