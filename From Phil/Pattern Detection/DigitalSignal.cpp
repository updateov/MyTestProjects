	...
	bool CFHRSignal::MultiHypothesis(void)
	{
		...
		try
		{
			if (m_ConfigFHR.m_bStoreXML)
			{
				...
				{
					fileFeature_Statistic.Open(CString(m_ConfigFHR.m_strXMLFilePrefix.c_str()) + "_MH_Feature_Statistic_Values.txt", CFile::modeCreate | CFile::modeWrite | CFile::typeText);
				}
				else
				{
					fileFeature_Statistic.Open
						(
						CString(m_ConfigFHR.m_strXMLFilePrefix.c_str()) + "_MH_Feature_Statistic_Values.txt",
	...
	void CFHRSignal::ClassifyDecels(fhrPartSet *d)
	{
		...
		if (toFile)
		{
			try
			{
				...
				fileFeature_Statistic.Open(CString(m_ConfigFHR.m_strXMLFilePrefix.c_str()) + "BumpMeasures_Decel_" + buff + ".txt", CFile::modeCreate | CFile::modeWrite | CFile::typeText);
			}
			catch(...)
			{
			}
	...
	void CFHRSignal::ClassifyDecel(decel *d, void *pFile)
	{
		...
		if ((pFile) && (!(d->isPending())))
		{
			...
			for (int iField = 0; iField < 20; iField++)
			{
				...
				{
					sFeature_Stat.Format("NaN\t");
				}
				else
				{
					sFeature_Stat.Format("%f\t", inVector(iField));
				}
	...
	void CFHRSignal::ClassifyAccels(fhrPartSet *accels, fhrPartSet *decels)
	{
		...
		try
		{
			if (toFile)
			{
				...
				fileFeature_Statistic.Open(CString(m_ConfigFHR.m_strXMLFilePrefix.c_str()) + "BumpMeasures_Accel_" + buff + ".txt", CFile::modeCreate | CFile::modeWrite | CFile::typeText);
			}
		}
			...
			try
			{
				if ((pFile) && (!(d->isPending())))
				{
					...
					for (int iField = 0; iField < 3; iField++)
					{
						...
						{
							sFeature_Stat.Format("%f\t", outVector(iField));
						}
						sFeature_Statistic += sFeature_Stat;
					}

					sFeature_Stat.Format("%d\t%d\n", d->getX1(), d->getX2());
					sFeature_Statistic += sFeature_Stat;
					((CStdioFile *) pFile)->WriteString(sFeature_Statistic);
				}
			}
...
*/
	...
	double CFHRSignal::Calculate_FeaturesPrimary(baseline *pBL[5], void *pFile)
	{
		...
		try
		{
			if (pFile)
			{
				...
				sFeature_Statistic.Format("%d\t%d\t1\t%d\t%f\t", pBL[2]->getX1() + m_OutputAbsStart, pBL[2]->getX2() + m_OutputAbsStart, m_nTotalCount, dRes);
				for (int iField = 0; iField < 55; iField++)
				{
					sFeature_Stat.Format("%f\t", vFHR_BL(iField));
					sFeature_Statistic += sFeature_Stat;
				}
				...
				for (int i = 0; i < 5; i++)
				{
					sFeature_Stat.Format("%d\t", pBL[i]->midpt());
					sFeature_Statistic += sFeature_Stat;
				}
	...
	double CFHRSignal::Calculate_FeaturesSecondary(baseline *pBL[5], void *pFile)
	{
		...
		try
		{
			if (pFile)
			{
				...
				sFeature_Statistic.Format("%d\t%d\t2\t%d\t%f\t", pBL[4]->getX1() + m_OutputAbsStart, pBL[4]->getX2() + m_OutputAbsStart, m_nTotalCount, dRes);
				for (int iField = 0; iField < 55; iField++)
				{
					sFeature_Stat.Format("%f\t", vFHR_BL(iField));
					sFeature_Statistic += sFeature_Stat;
	...
	*/
	ContractionDetection *CFHRSignal::CreateContractionDetection(void)
	{
		...
		m_ContractDetect = new ContractionDetection[1]; //PAW 20171122
