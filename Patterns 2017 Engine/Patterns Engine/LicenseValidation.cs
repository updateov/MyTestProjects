...
namespace PeriGen.Patterns.Engine
{
	...
	public static class LicenseValidation
	{
		...
		static LicenseValidation()
		{
			...
			{
				PatternsProcessorWrapper.Source.TraceEvent(TraceEventType.Error, 1124, "Unable to initialize the license engine.\n\n{0}", ex);
		...
		public static void EnableDemoMode()
		{
			if (LicenseInitialized)
			{
				...
				{
					PatternsProcessorWrapper.Source.TraceEvent(TraceEventType.Error, 1125, "Unable to set the license to demo mode!\n\n{0}", error);
				}
			}
		}
	}
